...
def odds(table, my_cards, num_oponents):
	...
	if table_togo != 0:
		table = table+ [Card(random.randint(0,12), random.randint(0,3)) for i in range(table_togo)]
	my_hand =table+my_cards
	my_score = hand.hand_value(my_hand)

	his_score = 0
	for i in range(num_oponents):
		his_cards = [Card(random.randint(0,12), random.randint(0,3)) for i in range(2)]
		his_hand = sorted(table + his_cards, key=attrgetter('value'))
		temp_score = hand.hand_value(his_hand)
