...
def load_dist(hand_cards, rtype='blind'):
	...
	if len(IDinds)>1:
		...
		return np.asarray(Darray[IDinds[0]]), bins, np.asarray(Dopparray[IDoppinds[0]]), bins
...
def pl_integrand(xbins, dist, bins, ibin):
	...
	#print(dist_trunc)
	bin_trunc = bins_mid[bins_mid<xbins[ibin+1]]
...
def odds_winning(own_dist, own_bins, opp_dist, opp_bins):
	...
	integrand = np.asarray([own_dist[ib]*pl_integrand(own_bins, opp_dist, opp_bins, ib) for ib in range(len(own_dist))])
	...
	plt.show()
	prob_winning = np.trapz(integrand, own_bins_arr)
...
def hand_probs(own_dist, own_bins):
	...
	print(own_bins_arr.shape, own_dist.shape)
	#opp_bins_arr = (opp_bins[1:] + opp_bins[:-1])/2.
...
def dist(table, table_tot, cards, cards_tot, accuracy=10000, score_range=(0, max_score), nbins=10000, ret=2, del_cards=[]):
	new_deck = []
	new_table = []
	new_cards = []
	hand_tot = []
...
def data_fetcher_general(hands, tables, IDarray, IDfname, Datafname, nbins=360, acc=10000, del_cards=[]):
	...
	if not (os.path.exists(IDfname) and os.path.exists(Datafname)):
		...
		data_array = np.empty((0,nbins), float)
	else:
	...
	for ihnd in range(len(hand_set)):
		if not any(np.equal(IDarray[ihnd], ident_array).all(1)):
			Dt_prev=time.time()
			distr_array, bin_array = dist(tables[ihnd], 5, hands[ihnd], 2,accuracy=acc, score_range=(0,max_score), nbins=nbins, ret=1, del_cards= del_cards[ihnd])
...
def blinds():
	...
	data_fetcher_general(hands, tables, IDs, 'blindIDs.npy', 'blindData.npy', nbins=3042, acc=100000, del_cards=[])
	"""ihand = 0
	Dt_prev=0
	start_time = time.time()
...
def blinds_opp():
	...
	opp_hands = []
	print('Number of hands to analyse: {0}'.format(len(hands)))
	data_fetcher_general(opp_hands, tables, IDs, 'blind_oppIDs.npy', 'blind_oppData.npy', nbins=3042, acc=100000, del_cards=hands)
