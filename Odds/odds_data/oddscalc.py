...
def load_dist(hand_cards, rtype='blind'):
	if rtype=='blind':
		...
		Darray = np.load(scriptpath+'/blinds/blindData.npy')
		IDopp_array = np.load(scriptpath+'/blinds/blind_oppIDs.npy')
		Dopparray = np.load(scriptpath+'/blinds/blind_oppData.npy')
		IDinds = np.where((IDarray == searchID).all(axis=-1))[0]
		IDoppinds = np.where((IDopp_array == searchID).all(axis=-1))[0]
		Dopparray, oppbins= np.histogram(Dopparray[IDoppinds[0]], bins=int(max_score), range=(0, max_score), normed=True)
		Darray, bins = np.histogram(Darray[IDinds[0]], bins=int(max_score),range=(0, max_score), normed=True)
	...
	if len(IDinds)>1:
		...
		return Darray, bins, Dopparray, oppbins
...
def pl_short(own_bins, own_dist, opp_cumsum):
	...
	bins_diff_avg = np.cumsum(bins_diff)/float(len(bins_diff))


	print(pw, type(pw))
	print(np.sum(own_dist*bins_diff))
	if np.absolute(np.sum(own_dist*bins_diff)-1.0)>1e-5:
		print('Error in  own distribution calculation.')
		print(np.mean(bins_diff), np.std(bins_diff))
		print(np.mean(own_dist), np.std(own_dist))
		sys.exit()
	if np.absolute(opp_cumsum[-1]-1.0)>1e-5:
		print('Error in  opp distribution calculation.')
		print(opp_cumsum[-1])
		print(np.mean(bins_diff), np.std(bins_diff))
		print(np.mean(opp_cumsum), np.std(opp_cumsum))
		sys.exit()
	print(np.sum(opp_cumsum))

	cumsum_diff = (opp_cumsum-np.cumsum(own_dist))
	cumsum_diff_int = np.sum(cumsum_diff*own_dist*bins_diff)
	plt.plot(0.5+cumsum_diff)
	plt.axhline(0.5+cumsum_diff_int)
	#plt.axhline(0.5+np.median(cumsum_diff), color='r')
	#plt.axhline(0.5+cumsum_diff_int, color='g')
	#plt.plot(np.cumsum(opp_cumsum-np.cumsum(own_dist)))
	plt.show()

	return 0.5+cumsum_diff_int#np.sum(bins_diff_avg*opp_cumsum*own_dist*bins_diff)
...
def odds_winning(own_dist, own_bins, opp_dist, opp_bins):
	...
	opp_cumsum=np.cumsum(opp_dist)
	own_cumsum=np.cumsum(own_dist)
	...
	prob_winning=  pl_short(own_bins, own_dist, opp_cumsum)
	prob_losing = pl_short(opp_bins, opp_dist, own_cumsum)
...
def dist(table, table_tot, cards, cards_tot, accuracy=10000, score_range=(0, max_score), nbins=10000, ret=1, del_cards=[]):
	new_deck = []
	new_table = []
	new_cards = []
	hand_tot = []
...
def data_fetcher_general(hands, tables, IDarray, IDfname, Datafname, nbins=360, acc=10000, del_cards=[]):
	best_hand =  [Card(12,3), Card(11,3), Card(10,3), Card(9,3), Card(8,3)]
	max_score = hand.full_hand_best(best_hand)[1]
	...
	if not (os.path.exists(IDfname) and os.path.exists(Datafname)):
		...
		data_array = np.empty((0,acc), float)
	else:
		ident_array = np.load(IDfname)
		data_array = np.load(Datafname)
	print(len(hands))
	print(ident_array)
	print(IDarray)
	for ihnd in range(len(hand_set)):
		if not any(np.equal(IDarray[ihnd], ident_array).all(1)):
			Dt_prev=time.time()
			distr_array = dist(tables[ihnd], 5, hands[ihnd], 2,accuracy=acc, score_range=(0,max_score), nbins=nbins, ret=1, del_cards= del_cards[ihnd])
			#own_bins_arr = (bin_array[1:] + bin_array[:-1])/2.
			#plt.plot(own_bins_arr,distr_array)
			#plt.plot(own_bins_arr,data_array[-1])
			print(distr_array.shape)
			print(data_array.shape)
			data_array = np.append(data_array, np.array([distr_array]), axis=0)
			...
			#plt.show()
			np.save(Datafname.split('.')[0], data_array)
			np.save(IDfname.split('.')[0], ident_array)
			ihand+=1
			print('Hand {0}: {1} complete.'.format(ihand, hand_set[ihnd]))
			if ihand%10==0:
				shutil.copyfile(Datafname, Datafname+'.backup')
...
def blinds():
	...
	data_fetcher_general(hands, tables, IDs, 'blindIDs.npy', 'blindData.npy', nbins=int(max_score), acc=100000, del_cards=[])
	"""ihand = 0
	Dt_prev=0
	start_time = time.time()
...
def blinds_opp():
	...
	opp_hands = []
	print('Number of hands to analyse: {0}'.format(len(hands)))
	data_fetcher_general(opp_hands, tables, IDs, 'blind_oppIDs.npy', 'blind_oppData.npy', nbins=int(max_score), acc=100000, del_cards=hands)
